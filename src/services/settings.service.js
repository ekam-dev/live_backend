...
exports.addCategoryDB = async (title, tenantId,color_code) => {
    const conn = await getMySqlPromiseConnection();

    try {
        const sql = `
        INSERT INTO categories
        (title, tenant_id,color_code)
        VALUES (?, ?,?);
        `;

        const [result] = await conn.query(sql, [title, tenantId,color_code]);
        return result.insertId;
    } catch (error) {
        console.error(error);
        throw error;
...
exports.getCategoriesDB = async (tenantId) => {
    ...
    try {
        const sql = `
        SELECT id,title,color_code FROM categories
        WHERE tenant_id = ?;
        `;
...
exports.updateCategoryDB = async (id, title, tenantId,color_code) => {
    const conn = await getMySqlPromiseConnection();
    ...
    try {
        ...
        await conn.query(sql, [title,color_code, id, tenantId]);
        return;
    } catch (error) {
        console.error(error);
        throw error;
