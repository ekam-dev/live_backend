...
exports.createOrderDB = async (tenantId, cartItems, deliveryType, customerType, customerId, tableId, paymentStatus = 'pending', invoiceId=null) => {
  ...
  try {
    ...
    (order_id, item_id, variant_id, price, quantity, notes, addons,modifiers, tenant_id)
    VALUES ?
    `;

    await conn.query(sqlOrderItems, [cartItems.map((item)=>[orderId, item.id, item.variant_id, item.price, item.quantity, item.notes, item?.addons_ids?.length > 0 ? JSON.stringify(item.addons_ids):null, item?.selectedModifierItems?.length > 0 ? JSON.stringify(item?.selectedModifierItems) : null ,tenantId ])]);
