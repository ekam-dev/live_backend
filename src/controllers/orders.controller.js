const { getOrdersDB, updateOrderItemStatusDB, cancelOrderDB, completeOrderDB, getOrdersPaymentSummaryDB, createInvoiceDB, completeOrdersAndSaveInvoiceIdDB,getAllOrdersDB,updateOrderStatesDB,orderStatsDB } = require("../services/orders.service");
const {
  getPaymentTypesDB,
  getPrintSettingDB,
  getStoreSettingDB,
...
exports.getAllOrders = async(req,res)=>{
    try{
        ...
        const data = await Promise.all([getAllOrdersDB(req.user.tenant_id)]);
...
exports.payAndCompleteKitchenOrder = async (req, res) => {
  try {
    const tenantId = req.user.tenant_id;
    const { orderIds, subTotal, taxTotal, total, selectedPaymentType } = req.body
    ...
    // get invoice id
    const invoiceId = await createInvoiceDB(subTotal, taxTotal, total, date, selectedPaymentType, tenantId);
...
exports.updateOrderStates = async(req,res)=>{

   try{

         const response = await updateOrderStatesDB(req);
...
exports.orderStats = async(req,res)=>{
    ...
    try{
          ...
          const response = await orderStatsDB(req.user.tenant_id);
